...
import type { NovaflowConfig } from "../types/index.js";
...
export async function createChatModel(config: NovaflowConfig): Promise<BaseChatModel> {
  ...
  switch (provider) {
    case "anthropic": {
      ...
      // When a baseUrl is set (e.g. Azure AI Foundry), use azure-compatible headers
    ...
    case "azure-openai": {
      ...
      return new AzureChatOpenAI({ model, apiKey, azureOpenAIBasePath: baseUrl, temperature });
    }

    case "ollama": {
      const { ChatOllama } = await import("@langchain/ollama");
      return new ChatOllama({ model, baseUrl: baseUrl ?? "http://localhost:11434", temperature });
    }
