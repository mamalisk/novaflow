...
/**
 * Writes per-agent Markdown report files to <workspaceRoot>/.novaflow/reports/<runId>/.
 * The workspace root is passed in from the extension host (panel.ts / runner.ts).
 */
export function writeRunReport(state: NovaflowStateType, workspaceRoot: string): void {
  const reportDir = join(workspaceRoot, ".novaflow", "reports", state.runId);
  ...
  if (state.jiraTicket) write(reportDir, "01-fetch-jira.md", buildFetchJira(state));
  if (state.baOutput) write(reportDir, "02-business-analyst.md", buildBusinessAnalyst(state));
  if (state.testPlanOutput) write(reportDir, "03-test-analyst.md", buildTestAnalyst(state));
  if (state.implementationOutput) write(reportDir, "04-developer.md", buildDeveloper(state));
  if (state.devopsOutput) write(reportDir, "05-devops.md", buildDevOps(state));
  if (state.testResults) write(reportDir, "06-playwright-runner.md", buildPlaywrightRunner(state));
  if (state.finalReport) write(reportDir, "07-report-generator.md", buildReportGenerator(state));
}
...
function buildTestAnalyst(state: NovaflowStateType): string {
  ...
  if (tp.automatedTests.length > 0) {
    ...
    lines.push(`|---|---|---|`);
    for (const t of tp.automatedTests) lines.push(`| ${t.name} | ${t.type} | ${t.description} |`);
  }
  if (tp.manualTests.length > 0) {
    lines.push(``, `## Manual Tests`, ``);
    for (const t of tp.manualTests) {
      lines.push(`### ${t.name}`, ``, `**Steps:**`, ``);
      for (let i = 0; i < t.steps.length; i++) lines.push(`${i + 1}. ${t.steps[i]}`);
...
function buildDeveloper(state: NovaflowStateType): string {
  ...
  for (const c of impl.changes) lines.push(`- \`${c.path}\` (${c.action})`);
  return lines.join("\n");
}

function buildDevOps(state: NovaflowStateType): string {
  const d = state.devopsOutput!;
  const lines = [`# DevOps`, ``, `## Summary`, ``, d.summary, ``, `## Pipeline Changes`, ``];
  if (d.pipelineChanges.length === 0) {
    lines.push("No pipeline files changed.");
  } else {
    for (const c of d.pipelineChanges) lines.push(`- \`${c.path}\` (${c.action})`);
  }
  return lines.join("\n");
}
...
function buildPlaywrightRunner(state: NovaflowStateType): string {
  ...
  if (failed.length > 0) {
    ...
    for (const t of failed) lines.push(`- **${t.name}**${t.error ? ` — ${t.error}` : ""}`);
  }
  if (r.results.length > 0) {
    lines.push(``, `## All Results`, ``);
    lines.push(`| Test | Status | Duration |`);
    lines.push(`|---|---|---|`);
    for (const t of r.results) lines.push(`| ${t.name} | ${t.passed ? "PASS" : "FAIL"} | ${t.duration}ms |`);
  }
  return lines.join("\n");
}
...
function buildReportGenerator(state: NovaflowStateType): string {
  ...
  if (fr.mrUrl) lines.push(`**MR:** ${fr.mrUrl}`);
